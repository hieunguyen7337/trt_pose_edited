...
EPS = 1e-6
use_apm = True
scaler = torch.cuda.amp.GradScaler(enabled=use_apm)
...
class FocalL2Loss(torch.nn.Module):
    def __init__(self, thr=0.01, gamma=2, alpha=0.1, beta=0.02):
    ...
    def forward(self, pred, gt, mask=None):
        pred = pred.float()
        gt = gt.float()
        Sdt = torch.where(torch.ge(gt , self.thr), pred - self.alpha, 1 - pred - self.beta)
        loss = (pred - gt)**2 * (1 - Sdt)**self.gamma
        if mask is not None:
          mask = mask.float()
          loss *= mask
        return loss.mean()
...
if __name__ == '__main__':
    ...
    for epoch in range(config["epochs"]):
        ...
        for image, cmap, paf, mask in tqdm.tqdm(iter(train_loader)):
            ...
            optimizer.zero_grad()
            # with torch.cuda.apm.autocast(enabled=use_apm):
            cmap_out, paf_out = model(image)
            
            # cmap_mse = FocalL2Loss.forward(FocalL2Loss(),cmap_out,cmap,mask)
